...
- **Hugging Face architecture**: HF retired their old
  ...
  Which providers actually serve which models shifts constantly on HF's
  side, so this extension tries HF's own currently-documented "recommended
  models" for text-to-image (`FLUX.1-Krea-dev` → `Qwen-Image` →
  `Hyper-SD` → `FLUX.1-schnell`) across several providers (`hf-inference`
  → `fal-ai` → `together` → `replicate` → `nscale`), falling through
  automatically whenever a combination doesn't work. Some of these models
  are "gated" (need a one-time license click on the model's HF page while
  logged into the same account as your token) — the error message will
  link directly to the relevant page if every combination fails.
